...
function DashboardCard08() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch('https://assignment-1-fc7lwmf3uq-el.a.run.app/order-value');
        const jsonData = await response.json();
        setOrderValueData(jsonData);
      } catch (error) {
        console.error(error);
      }
    }
  ...
  const orderValueDataByRegion = orderValueData?.data?.values ?? [];
